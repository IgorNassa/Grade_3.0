...
public class AulaServiceImpl implements AulaService {
    ...
    private final br.sistema.model.repository.interfaces.TurnoRepository turnoRepository;

    public AulaServiceImpl(AulaRepository aulaRepository, ProfessorRepository professorRepository, br.sistema.model.repository.interfaces.TurnoRepository turnoRepository) {
    ...
    public void gerarGrade(Turma turma, Map<Disciplina, Integer> cargaHoraria, String turno) {
        ...
        // 2. Buscar informações do turno para definir os slots
        br.sistema.model.entity.Turno turnoEntidade = turnoRepository.findByName(turno);
        if (turnoEntidade == null) {
            throw new NotFoundException("Turno '" + turno + "' não encontrado.");
        }
        int totalSlots = turnoEntidade.getAulasTurno();

        // 3. Preparar e embaralhar horários para melhor distribuição
    ...
    private boolean possoAlocar(Professor professor, Turma turma, DayOfWeek dia, int slot, List<Aula> gradeAtual) {
        ...
        // Como o algoritmo é para uma turma por vez, se já houver aula nesse dia/slot, não pode alocar.
