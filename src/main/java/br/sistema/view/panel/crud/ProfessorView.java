...
public class ProfessorView extends JPanel {
    ...
    private final br.sistema.controller.interfaces.DisciplinaController disciplinaController;

    public ProfessorView() {
        this.professorController = ServiceRegistry.getInstance().get(ProfessorController.class);
        this.disciplinaController = ServiceRegistry.getInstance().get(br.sistema.controller.interfaces.DisciplinaController.class);
    ...
    private JPanel buildForm() {
        ...
        JPanel painelDisciplinas = new JPanel(new BorderLayout(5, 0));
        painelDisciplinas.setOpaque(false);
        txtDisciplinas = AppTheme.styledField("Ex: Matemática, Física", 300);
        
        JButton btnEscolher = new JButton("▼");
        btnEscolher.setPreferredSize(new Dimension(30, 38));
        btnEscolher.setBackground(AppTheme.BG_FIELD);
        btnEscolher.setForeground(AppTheme.TEXT_SECONDARY);
        btnEscolher.setFocusPainted(false);
        btnEscolher.setBorder(BorderFactory.createLineBorder(AppTheme.BORDER_COLOR));
        btnEscolher.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btnEscolher.addActionListener(e -> mostrarMenuDisciplinas(btnEscolher));
        
        painelDisciplinas.add(txtDisciplinas, BorderLayout.CENTER);
        painelDisciplinas.add(btnEscolher, BorderLayout.EAST);
        card.add(painelDisciplinas, gc);
    ...
    private void fecharFormulario() {
        // Agora o formulário não fica mais oculto, apenas limpamos o estado
        lblFormTitulo.setText("Cadastrar Novo Professor");
        resetFormulario();
        revalidate();
        repaint();
    ...
    private void mostrarMenuDisciplinas(JButton btn) {
        JPopupMenu menu = new JPopupMenu();
        menu.setBackground(AppTheme.BG_CARD);
        menu.setBorder(BorderFactory.createLineBorder(AppTheme.BORDER_COLOR));

        try {
            List<br.sistema.controller.dtos.DisciplinaDTO> lista = disciplinaController.findAll();
            if (lista.isEmpty()) {
                JMenuItem item = new JMenuItem("Nenhuma disciplina cadastrada");
                item.setEnabled(false);
                menu.add(item);
            } else {
                for (br.sistema.controller.dtos.DisciplinaDTO d : lista) {
                    JMenuItem item = new JMenuItem(d.nome());
                    item.addActionListener(e -> {
                        String atual = txtDisciplinas.getText().trim();
                        if (atual.isEmpty()) {
                            txtDisciplinas.setText(d.nome());
                        } else {
                            if (!Arrays.asList(atual.split(",\\s*")).contains(d.nome())) {
                                txtDisciplinas.setText(atual + ", " + d.nome());
                            }
                        }
                    });
                    menu.add(item);
            ...
            JMenuItem item = new JMenuItem("Erro ao carregar");
            menu.add(item);
        }
    ...
    private JTable buildTable() {
        ...
        t.getColumnModel().getColumn(3).setCellRenderer(AppTheme.statusRenderer());
