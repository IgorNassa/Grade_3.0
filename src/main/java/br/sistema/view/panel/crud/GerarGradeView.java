...
public class GerarGradeView extends JPanel {
    ...
    private JComboBox<br.sistema.controller.dtos.TurnoDTO> cmbTurno;
    ...
    private void gerarGrade() {
        ...
        int limite = (turnoDTO != null) ? turnoDTO.aulasTurno() * 5 : 25;
        if (totalAulas > limite) {
            JOptionPane.showMessageDialog(this, "A grade do turno " + nomeTurno + " comporta no máximo " + limite + " aulas semanais. Reduza a carga.", "Erro", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // --- DELEGA PARA O SEU BACKEND FAZER O TRABALHO PESADO ---
        try {
            aulaController.gerarGrade(turmaSelecionada, cargaParaGerar, nomeTurno);
        } catch (Exception ex) {
