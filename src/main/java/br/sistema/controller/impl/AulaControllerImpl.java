...
public class AulaControllerImpl implements AulaController {
    ...
    @Override
    public void gerarGrade(TurmaDTO turmaDTO, Map<DisciplinaDTO, Integer> cargaHoraria, String turno) {
        ...
        aulaService.gerarGrade(turma, mapEntity, turno);
    }

    @Override
    public void deleteByTurma(TurmaDTO turmaDTO) {
        if (turmaDTO == null) return;
        Turma turma = new Turma();
        turma.setId(turmaDTO.id());
        aulaService.deleteByTurma(turma);
    }
}